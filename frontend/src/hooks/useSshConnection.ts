 ...
 * @param {Function} props.onOpenTerminal - 在内部连接成功后用于打开新终端会话的回调函数。
 * @returns {{ connect: (alias: string, type?: 'local' | 'remote', sessionID?: string, strategy: 'internal' | 'external' = 'external') => void }} - 返回一个包含 connect 函数的对象。
 */
            ...
            try {
              ...
              if (result.success) {
                if (strategy === 'internal') {
                  ...
                  switch (type) {
                    case 'local':
                      sessionInfo = await StartLocalSession(sessionID as string)
                      break
                    case 'remote':
